...
`dataset.jsonl` is generated from the 30 source files -- never edit it directly. Edit the source JSONL, then rebuild. (`eval.jsonl` is also regenerated by the rebuild, but unlike `dataset.jsonl` it may be hand-edited to hold truly held-out records that exist in no source file.)

```bash
python build.py
```

The split is grouped by prompt (not by record index), so the same question never appears in both training and evaluation. Selection is by SHA-1 hash of the prompt, so the split is deterministic and stable: editing one record does not reshuffle the rest. `eval.jsonl` is regenerated by the rebuild, so hand-written eval records that exist in no source file will be overwritten -- add them to a source file first, or manage `eval.jsonl` outside the rebuild.
