...
### Rebuild the derived files after any source edit

`dataset.jsonl` is generated from the 30 source files -- never edit it directly. Edit the source JSONL, then rebuild. (`eval.jsonl` is hand-curated held-out data that exists in no source file; it is NOT regenerated by the rebuild, so editing it directly is the correct way to manage the evaluation set.)

```bash
python build.py
```

The split is grouped by prompt (not by record index), so the same question never appears in both training and evaluation. `eval.jsonl` is hand-curated separately and not overwritten by the rebuild; `validate.py` enforces that no prompt appears in both `dataset.jsonl` and `eval.jsonl`, which is the held-out guarantee.
...
`dataset.jsonl` is **generated** from the source library, never edited directly. See [`AGENTS.md`](AGENTS.md) for the full rebuild procedure.

`eval.jsonl` is **hand-curated held-out data** and is **not regenerated** by `build.py`. Edit it directly -- it holds records that exist in no source file, making it truly held-out evaluation data the model has never seen. `validate.py` enforces that no prompt appears in both files.
